...
if config.HAVE_FENICS:
    ...
    class FenicsOperator(OperatorBase):
        ...
        @defaults('restriction_method', qualname='pymor.bindings.fenics.FenicsOperator.__init__')
        def __init__(self, form, source_function, dirichlet_bc=None,
            ...
            self.source = FenicsVectorSpace(source_function.function_space())
            self.range = FenicsVectorSpace(form.arguments()[0].function_space())
            self.form = form
        ...
        def restricted(self, dofs):
            ...
            if self.restriction_method == 'assemble_local':
                ...
                subdomain = df.CellFunction('size_t', mesh)
                for ci in affected_cell_indices:
                ...
                args = tuple((df.Argument(V_r_range, arg.number(), arg.part())
                ...
                form_r = df.replace_integral_domains(
                ...
                op_r = FenicsOperator(form_r, source_function_r, dirichlet_bc=bc_r,
                                      parameter_setter=self.parameter_setter, parameter_type=self.parameter_type)
