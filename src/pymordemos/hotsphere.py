...
auxNames = ['nxi','nyi','nzi','gi','nxj','nyj','nzj','gj','nxk','nyk','nzk','gk','kappa','dr','r','gamman','g_r']
gravityTerm = True
...
xyzMin = [theta0-0.15, phi0-0.15, radiusOfEarth + 80.0e5 ] # Domain lower limits
...
if __name__=="__main__":
    ...
    #q  = np.reshape(problem.claw.solution.state.q, problem.claw.solution.state.q.size, order='F')
    ...
    visualizer = PyClawVisualizer(fom)

    visualizer.visualize(U,math.floor(num_output_times) - 1)
    """

    """
    fom.visualize(U, math.floor(num_output_times) - 1)  # plot the solution
