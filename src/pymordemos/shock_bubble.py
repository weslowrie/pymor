...
if __name__=="__main__":
    ...
    fom = setup(kernel_language='Fortran', solver_type='classic', use_petsc=False,
          dimensional_split=False, outdir='_output', output_format='hdf5',
          disable_output=False, num_cells=(256, 64, 32),
          tfinal=0.3, num_output_times=100)
    U = fom.solve()
    modes, svals = pod(U)
    ...
    fom.visualize(U, 70)
    fom.visualize(U, 90)
    fom.visualize(U,num_output_times-1)

    from pymor.algorithms.basic import *
    Up = project_array(U, modes[0:3])
    fom.visualize(Up, num_output_times-1)

    Up = project_array(U, modes[0:50])
    fom.visualize(Up, num_output_times - 1)

    Up = project_array(U, modes[0:70])
    fom.visualize(Up, num_output_times - 1)

    Up = project_array(U, modes[0:90])
    fom.visualize(Up, num_output_times - 1)
