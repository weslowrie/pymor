...
def setup_requires(toml=False):
    other = ['setuptools>=40.8.0', 'wheel', 'pytest-runner>=2.9', _CYTHON, 'packaging',]
    if toml:
        return [f.replace('numpy>=', 'numpy==') for f in _NUMPYS] + other
    return _NUMPYS + other

tests_require = [_PYTEST, 'pytest-cov', 'envparse', 'docker']
install_requires = ['scipy>=0.13.3', 'Qt.py', 'packaging', _SPHINX,'diskcache'] + setup_requires()
...
doc_requires = [_SPHINX, _CYTHON, 'nbsphinx', 'nbsphinx_link'] + _NUMPYS
